...
GdkPixbuf* get_icon_from_filename(const gchar* uri, GtkIconTheme* icon_theme)
{
    if(g_file_test(uri, G_FILE_TEST_IS_DIR)) return gtk_icon_theme_load_icon(icon_theme, "folder", 48, 0, NULL);
		...
		GFileInfo *file_info = g_file_query_info(file, "*", 0, NULL, NULL);
		gchar *mime = (gchar *)g_file_info_get_content_type(file_info);
		strchr(mime, '/')[0] = '-';
		if (gtk_icon_theme_has_icon(icon_theme, mime)) {
			return gtk_icon_theme_load_icon(icon_theme, mime, 48, 0, NULL);
		}
	}
	return gtk_icon_theme_load_icon(icon_theme, "gtk-file", 48, 0, NULL);
}
